...
qda.fit = qda(BANKRUPTCY ~ ., data=data, subset=train)
...
train.data = data[train,]
test.data = data[test,]
qda.fit = qda(BANKRUPTCY ~ ., data=train.data)


names(data)
qda.fit = qda(BANKRUPTCY ~ AGE + DEBT_COST + RETURN_EQUITY + RETURN_ASSETS + SPENT_JUDICIAL_INCIDENCES_TOTAL + AUDITED, data=data, subset=train)
qda.pred = predict(qda.fit, test.data, type="response")
mean(qda.pred$class != test.data$BANKRUPTCY) # 0.01845638


#######

glm(BANKRUPTCY ~ SIZE + DEBT_COST + RETURN_EQUITY + RETURN_ASSETS + NUMBER_JUDICIAL_INCIDENCES_YEAR + SPENT_JUDICIAL_INCIDENCES_TOTAL + AUDITED, data=data, family = "binomial")
